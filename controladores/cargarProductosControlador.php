<?php
	if($peticionAjax){
		require_once "../modelos/cargarProductosModelo.php";
	}else{
		require_once "./modelos/cargarProductosModelo.php";
	}

	class cargarProductosControlador extends cargarProductosModelo
	{
		...
		public function cargar_lista_productos($productos)
		{
			...
			{
				$infoProducto = cargarProductosModelo::obtener_info_producto_modelo($producto["sku"]);
		...
		public function obtener_precio_producto($producto, $tipo, $precio)
		{
			...
			$categoria = cargarProductosModelo::obtener_categoria_modelo($producto);
			if ($categoria->rowCount()>0)
			{
				$categoria = $categoria->fetch();
				$reglaProducto = cargarProductosModelo::obtener_regla_modelo($categoria['id_taxonomia'], $tipoUsuario);
				...
				$descuentoProductoSku = cargarProductosModelo::verificar_descuento_modelo($producto, "producto");
				if ($descuentoProductoSku->rowCount()>0)
				{
					$datosDescuento = $descuentoProductoSku->fetchAll();
					foreach ($datosDescuento as $descuento)
					{
						$infoDescuento = cargarProductosModelo::obtener_info_descuento_modelo($descuento['id_descuento'], $tipoUsuario);
				...
				$relaciones = cargarProductosModelo::obtener_relaciones_modelo($producto);
				if ($relaciones->rowCount()>0)
				{
					$relaciones = $relaciones->fetchAll();
					foreach ($relaciones as $relacion)
					{
						$infoRelacion = cargarProductosModelo::obtener_info_relacion_modelo($relacion['id_taxonomia']);
						if ($infoRelacion->rowCount()>0)
						{
							$infoRelacion = $infoRelacion->fetch();
							$descuentoProductoRelacion = cargarProductosModelo::verificar_descuento_modelo($relacion['id_taxonomia'], $infoRelacion['taxonomia']);
