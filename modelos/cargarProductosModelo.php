<?php
	...
	class cargarProductosModelo extends mainModel
	{
		
		protected function obtener_info_producto_modelo($sku){
			$sql=mainModel::conectar()->prepare("SELECT sku, nombre, slug, precio, calificacion FROM productos WHERE sku=:Sku");
			$sql->bindParam(":Sku",$sku);
			$sql->execute();
			return $sql;
		}

		protected function obtener_categoria_modelo($sku){
